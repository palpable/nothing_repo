class Admin::PreferencesController < ApplicationController
  ...
  def index
     @pendings = Person.find_by_sql("select * from people where active=false and deactivated=false and isbanker=true")
     if @pendings.size > 0
       flash[:error] ="#{@pendings.size} users are waiting for your approval"
     end
     render :action => "show"
  end
  ...
  def edit
    @business_types = AdminSetting.all(:conditions => {:title => 'business_type'})
  end

  def update    
    respond_to do |format|
      old_preferences = @preferences.clone
      business_types = params[:preferences][:business_types].split(",").collect{|business_type| business_type.strip}
      
      AdminSetting.update_business_types(business_types)
