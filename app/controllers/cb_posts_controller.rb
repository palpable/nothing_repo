class CbPostsController < ApplicationController
...
# There is some trickery to handle the two in a unified manner.
  ...
  def index
    ...
    redirect_to cb_forum_cb_topic_url(@cb_forum, @topic) if forum?
  end
  ...
  private
    ...
    def get_instance_vars
      ...
      if forum?
        @cb_forum = CbForum.find(:first)
        @topic = CbTopic.find(params[:cb_topic_id])
        @body = "forum"
      elsif blog?
    ...
    def resource_posts
      if forum?
        ...
        @blog.cb_post.paginate(:page => params[:page])
      end
    end
    ...
    def new_resource_post
      if forum?
        post = @topic.cb_post.build(params[:cb_post])
        post.person = current_person
      elsif blog?
        post = @blog.cb_post.new(params[:cb_post])
      end
      post
    end
    ...
    def post_url
      if forum?
        ...
        cb_forum_cb_topic_url(@cb_forum, @topic, :posts => @topic.cb_post.count)
      elsif blog?
    ...
    def posts_url
      if forum?
       cb_forum_cb_topic_url(@cb_forum, @topic)
      elsif blog?
        blog_tab_url(@blog)
