module ActivitiesHelper
  ...
  def feed_message(activity, recent = false)
    ...
    when "ForumPost"
      ...
      post = activity.item
      if recent
        %(new post to forum topic #{cb_topic_link(post.cb_topic)})
      else
        %(#{person_link_with_image(person)} made a post to forum topic
          #{cb_topic_link(post.cb_topic)})
      end
    ...
    when "Topic"
     ...
     when "CbTopic"
      if recent
        %(new discussion topic #{cb_topic_link(activity.item)})
      else
        %(#{person_link_with_image(person)} created the new discussion topic
          #{cb_topic_link(activity.item)})
      end
  ...
  def cb_topic_link(text,topic=nil)
    if topic.nil?
      topic = text
      text = topic.name
    end
    link_to(text, cb_forum_cb_topic_path(topic.cb_forum, topic))
  end
