...
class Person < ActiveRecord::Base
  ...
  attr_accessor :password, :verify_password, :new_password,
                :sorted_photos, :email_confirmation, :firstname, :lastname
  attr_accessible :email, :email_confirmation, :password, :password_confirmation, :name,
                  ...
                  :blog_comment_notifications, :identity_url, :branch, :isbanker, :years, 
		              :department, :business_type, :company, :phone, :fax, :country, :title, :firstname, :lastname,
		              :phone_visibility, :email_visibility

  # Indexed fields for Sphinx
  is_indexed :fields => [ 'name', 'deactivated',
                          'email_verified'],
             :conditions => "deactivated = false AND (email_verified IS NULL OR email_verified = true)"

  BUSINESS_TYPES = AdminSetting.all(:conditions => {:title => 'business_type'}).collect(&:value)
#[
#"Bank",
#"Asset Manager",
#"Hedge Fund Manager",
...
#]
  ...
  has_one :blog
  has_one :news_feed,:dependent => :destroy, :order => 'created_at'
...
#  validates_presence_of     :email, :name #, :title, :branch, :years
  ...
  validates_length_of       :name,  :maximum => MAX_NAME, :message => "Too long with name #{name}" 
  validates_length_of       :description, :maximum => MAX_DESCRIPTION
  validates_format_of       :email,
                            :with => EMAIL_REGEX,
                            :message => "must be a valid email address"
  validates_uniqueness_of   :email
# Fairly generous phone number validation -- any digit and any extension 

	#validates_format_of :phone, :fax,
	#:message => "must be a valid telephone number.",
    	#:with => /^[\(\)0-9\- \+\.]{10,20} *[extension\.]{0,9} *[0-9]

  validates_uniqueness_of   :identity_url, :allow_nil => true


  before_create :create_blog, :check_config_for_deactivation, :make_invitation_code
  before_validation_on_create :combine_names
  ...
  class << self
    ...
    def mostly_active(page = 1)
      paginate(:all, :page => page,
                     ...
                     :order => "SUBSTRING(name, INSTR(name,' ') + 1) ASC")
    end
  ...
  def recent_activity(sort = 'DESC')
    return nil unless sort =~ /^ASC|DESC$/i
    Activity.find_all_by_person_id(self, :order => "created_at #{sort}",
                                         :limit => FEED_SIZE)
  end
  ...
  def self.authenticate(email, password)
    #return Person.first # bypass crypto for devel
  ...
  def decrypt(password)
    #return # bypass crypto for devel
  ...
  def make_invitation_code
    #puts "******************************************* Make invitation_code Called ***********************************"
    self.invitation_code =UUID.new
  end
